...
(function () {
  ...
  const TYPE_COLOR = {
    discovery: "#2563eb", feature: "#00B67A", bugfix: "#dc2626",
    change: "#d97706", decision: "#7c3aed", refactor: "#6b7280", summary: "#7c3aed",
  };
  const SESSION_COLOR = "#00B67A", FILE_COLOR = "#9ca3af", CONCEPT_COLOR = "#d97706";

  const state = {
    query: "", project: "all", kind: "all", mode: "keyword",
    selectedRecId: null, selectedSessId: null,
  };
  let meta = null;
  ...
  async function init() {
    ...
    search.addEventListener("input", () => {
      clearTimeout(t);
      t = setTimeout(
        () => { state.query = search.value.trim(); state.selectedSessId = null; refresh(); },
  ...
  async function refresh() {
    ...
    const params = new URLSearchParams({
      q: state.query, project: state.project, kind: state.kind, mode: state.mode, limit: "250",
    });
    if (state.selectedSessId && !state.query) params.set("session", state.selectedSessId);
    ...
    setStatus(
      data.mode === "semantic" ? `semantic · top ${data.total}`
        : data.mode === "indexing" ? "building semantic index… (keyword for now)" : "",
    );
  ...
  function renderResults(recs) {
    ...
    if (!recs.length) {
      list.appendChild(el("div", "empty", "No matches. Try a different term or widen the project filter."));
      return;
    }
    ...
    recs.forEach((r) => {
      ...
      row.appendChild(el("div", "result-meta",
        `${r.project} · ${(r.date || "").slice(0, 10)}` + (r.files.length ? ` · ${r.files.length} file(s)` : "")));
  ...
  function renderDetail() {
    ...
    if (!r) {
      ...
      d.textContent = "Select a result or a node to see its full content, files, concepts, and how to resume its session.";
      return;
    }
    d.className = "detail";
    d.innerHTML = "";
    ...
    if (r.date) row.appendChild(el("span", "cchip", r.date.slice(0, 16).replace("T", " ")));
    d.appendChild(row);
    if (r.subtitle) {
      const s = el("p", "body", r.subtitle);
      s.style.fontWeight = "600"; s.style.color = "var(--text)";
      d.appendChild(s);
    }
    ...
    if (r.concepts && r.concepts.length) {
      ...
      r.concepts.forEach((c) => {
        ...
        chip.addEventListener("click", () => { $("#search").value = c; state.query = c; clearSelection(); refresh(); });
  ...
  function selectRecord(id) {
    ...
    document.querySelectorAll(".result").forEach((x) => x.classList.toggle("sel", x.dataset.id === id));
  ...
  // ===================== GRAPH =====================
  let sim = null, gZoom = null, svgSel = null, rootG = null;

  function buildSubgraph(recs) {
    const focus = recs.slice(0, 70);
    const nodes = new Map(), links = [];
    const addNode = (id, o) => { if (!nodes.has(id)) nodes.set(id, Object.assign({ id }, o)); return nodes.get(id); };

    new Set(focus.map((r) => r.sessionId).filter(Boolean)).forEach((sid) => {
      const s = sessByContentId.get(sid);
      addNode("S:" + sid, { kind: "session", label: s ? truncate(s.title, 28) : "session", color: SESSION_COLOR, r: 9, ref: sid });
    });
    focus.forEach((r) => {
      addNode(r.id, { kind: r.kind, label: truncate(r.title, 26), color: TYPE_COLOR[r.type] || "#6b7280", r: 5.5, ref: r.id });
      if (r.sessionId && nodes.has("S:" + r.sessionId)) links.push({ source: "S:" + r.sessionId, target: r.id, kind: "owns" });
    });
    addSharedConnectors(focus, "files", "F:", FILE_COLOR, baseName, nodes, links, addNode, 25);
    addSharedConnectors(focus, "concepts", "C:", CONCEPT_COLOR, (x) => x, nodes, links, addNode, 25);
    return { nodes: [...nodes.values()], links };
  }

  function addSharedConnectors(focus, field, prefix, color, labelFn, nodes, links, addNode, cap) {
    const usage = new Map();
    focus.forEach((r) => (r[field] || []).forEach((v) => {
    ...
    [...usage.entries()]
      ...
      .forEach(([v, rs]) => {
        const id = prefix + v;
        addNode(id, { kind: field === "files" ? "file" : "concept", label: truncate(labelFn(v), 22), color, r: 4 });
        rs.forEach((rid) => links.push({ source: id, target: rid, kind: field }));
      });
  }
  ...
  function renderGraph(recs) {
    ...
    if (!svgSel) {
      ...
      gZoom = d3.zoom().scaleExtent([0.2, 4]).on("zoom", (e) => rootG.attr("transform", e.transform));
      svgSel.call(gZoom);
    }
    svgSel.attr("viewBox", `0 0 ${W} ${H}`);
    rootG.selectAll("*").remove();
    if (!g.nodes.length) { if (sim) sim.stop(); return; }

    const link = rootG.append("g").selectAll("line").data(g.links).enter()
      .append("line").attr("class", "glink")
      .attr("stroke-width", (d) => (d.kind === "owns" ? 1.3 : 0.8))
      .attr("stroke-dasharray", (d) => (d.kind === "files" || d.kind === "concepts" ? "3,3" : null));

    const node = rootG.append("g").selectAll("g").data(g.nodes).enter()
      .append("g").attr("class", "gnode").attr("data-id", (d) => d.ref || d.id);
    node.append("circle").attr("r", (d) => d.r).attr("fill", (d) => d.color)
      .attr("stroke", (d) => d3.color(d.color).darker(0.6));
    node.append("text").attr("x", (d) => d.r + 3).attr("dy", "0.32em").text((d) => d.label);
    ...
    sim = d3.forceSimulation(g.nodes)
      .force("link", d3.forceLink(g.links).id((d) => d.id).distance((d) => (d.kind === "owns" ? 38 : 60)).strength(0.4))
  ...
  function onNodeClick(d) {
    ...
    if (d.kind === "file") { const v = baseName(d.id.slice(2)); $("#search").value = v; state.query = v; clearSelection(); return refresh(); }
    if (d.kind === "concept") { const v = d.id.slice(2); $("#search").value = v; state.query = v; clearSelection(); return refresh(); }
    selectRecord(d.ref);
  }
  ...
  function highlightGraph(recId) {
    ...
    rootG.selectAll(".glink").each(function (d) {
      const s = nodeRef(d.source.id || d.source), t = nodeRef(d.target.id || d.target);
      if (s === recId || t === recId) { neighbors.add(s); neighbors.add(t); }
    });
    rootG.selectAll(".gnode")
  ...
  function buildLegend() {
    const items = [
      ["session", SESSION_COLOR], ["discovery", TYPE_COLOR.discovery], ["feature", TYPE_COLOR.feature],
      ["bugfix", TYPE_COLOR.bugfix], ["change", TYPE_COLOR.change], ["decision", TYPE_COLOR.decision],
      ["summary", TYPE_COLOR.summary], ["file", FILE_COLOR], ["concept", CONCEPT_COLOR],
    ];
    const leg = $("#legend");
    items.forEach(([label, color]) => {
      ...
      dot.style.borderColor = d3.color(color).darker(0.6);
      it.appendChild(dot);
  ...
  function connectSSE() {
    ...
    es.addEventListener("ping", (e) => {
      const m = JSON.parse(e.data);
      if (m.indexing && !$("#sem-status").textContent) setStatus(`indexing semantic… (${m.indexed})`);
    });
  ...
  function flashFresh() {
    ...
    setTimeout(() => { if (s.textContent === "↻ updated with new entries") s.textContent = prev; }, 2500);
  }
